 ...
 */
export const fetchIntegrationByUserAndAgent = async (userId: string, agentName: string): Promise<{id: string, api: string, token: string, timezone?: string, approver?: number, senderName?: string, senderEmail?: string} | null> => {
  try {
    ...
    if (response.data && response.data.records && response.data.records.length > 0) {
      ...
      return {
        ...
        senderName: record.fields.remetente || '',
        senderEmail: record.fields.email || ''
      };
    }
